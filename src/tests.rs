...
unsafe fn main_0() -> libc::c_int {
    ...
    loop {
        ...
        if hash[..] != blake2b_keyed_kat[i as usize][..BLAKE2B_OUTBYTES as usize] {
    ...
    match current_block {
        ...
        /* Test streaming API */
        {
            ...
            's_72: loop {
                if !(step < BLAKE2B_BLOCKBYTES as libc::c_int as libc::c_ulong) {
                ...
                while i < 256 as libc::c_int as libc::c_ulong {
                    ...
                    let mut S: blake2b_state = blake2b_state {
                        h: [0; 8],
                        t: [0; 2],
                        f: [0; 2],
                        buf: [0; 128],
                    ...
                    while mlen >= step {
                        err = blake2b_update(&mut S, p as *const libc::c_void, step);
                        if err < 0 as libc::c_int {
                            current_block = 8536108021738834262;
                            break 's_72;
                        }
                        mlen = (mlen as libc::c_ulong).wrapping_sub(step) as size_t as size_t;
                        p = p.offset(step as isize)
                    }
                    err = blake2b_update(&mut S, p as *const libc::c_void, mlen);
                    if err < 0 as libc::c_int {
                        current_block = 8536108021738834262;
                        break 's_72;
                    }
                    err = blake2b_final(
                        &mut S,
                        hash_0.as_mut_ptr() as *mut libc::c_void,
                        BLAKE2B_OUTBYTES as libc::c_int as size_t,
                    );
                    if err < 0 as libc::c_int {
                        current_block = 8536108021738834262;
                    ...
                    if hash_0[..] != blake2b_keyed_kat[i as usize][..BLAKE2B_OUTBYTES as usize] {
                        current_block = 8536108021738834262;
                        break 's_72;
                    }
                    i = i.wrapping_add(1)
